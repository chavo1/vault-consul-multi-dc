...
Vagrant.configure(2) do |config|
    ...
    # 'a' count the DCs (dc names) 'b' count the IPs (increase with 1)
    ...
    1.upto(SERVER_COUNT) do |n|
      config.vm.define "consul-#{a}-server0#{n}" do |server|
        ...
        server.vm.provision "shell",inline: "cd /vagrant ; bash scripts/consul.sh", env: {"DOMAIN" => DOMAIN, "CONSUL_VERSION" => CONSUL_VERSION, "SERVER_COUNT" => SERVER_COUNT}
    ...
    1.upto(CLIENT_COUNT) do |n|
      config.vm.define "consul-#{a}-client0#{n}" do |client|
        ...
        client.vm.provision "shell",inline: "cd /vagrant ; bash scripts/consul.sh", env: {"DOMAIN" => DOMAIN, "CONSUL_VERSION" => CONSUL_VERSION, "CLIENT_COUNT" => CLIENT_COUNT}
